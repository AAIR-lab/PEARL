...
class GoalEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if self.time == self.max_time:
            ...
            # reward = reward * 10
            return (state, 0), reward, end_episode, info
        end_episode = False
        run = True
        reward = 0.
        while run:
            steps += 1
            reward, end_episode, success = self._update(act, param)
            run = not end_episode
            if run:
                ...
                if act == DRIBBLE:
                    ...
                    run = False
        state = self.get_state()
        info["success"] = success # NEW: added success
        # reward = reward * 10
        return (state, steps), reward, end_episode, info
    ...
    def get_state(self):
        ...
        state = np.minimum(np.maximum(state, LOW_VECTOR), HIGH_VECTOR)
    ...
    def _terminal_check(self):
        ...
        if end_episode:
            self.states.append([
                ...
                self.goalie.orientation,
                self.ball.position.copy()])
        return reward, end_episode, success
